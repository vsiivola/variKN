...
int main(int argc, char **argv) {
  ...
  config("Usage: varigram_kn [OPTIONS] textin LM_out\nProduces variable span n-gram from given text\n")
    ...
    ('3', "3nzer","","","Use 3 discounts per order instead of one. Takes a bit more memory during the model estimation.")
    ('f',"nfirst=INT", "arg", "-1", "Number of most common words to be included")
    ...
    ('Z', "dontaddzpg", "", "", "This flag prevents the addition of zeroprob grams to fill out the treegram structure.")
  ...
  if (vocabout.size()) vocab_outf.open(vocabout, "w");
  if (countsout.size()) counts_outf.open(countsout, "w");
  ...
  try {
    if (config["clear_history"].specified)
      vg->initialize(infilename, hashs, ndrop, nfirst, optiname,
		     "<s>", smallmem, vocabname);
    else 
      vg->initialize(infilename, hashs, ndrop, nfirst, optiname, 
		     "", smallmem, vocabname);
    ...
    if (!narpa) {
      ...
      if (zpg) vg->set_zeroprobgrams(false);
