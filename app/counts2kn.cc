...
int main(int argc, char **argv) {
  ...
  config("Usage: counts2kn [OPTIONS] text_in lm_out.\nCreates an interpolated KN language model based on the give counts\n")
    ...
    ('3', "3nzer","","","Use 3 discounts per order instead of one. Takes a bit more memory during model estimation.")
    ('B', "vocabin=FILE", "arg", "", "Input count file contains word indices. This is the corresponding vocabulary")
    ('H', "hashsize=INT", "arg", "0", "Size of the reserved hash table. Speed vs. memory consumption.")
    ('c', "counts","","","The first arguments is a counts file with already KN-smoothed counts, not a text file to be read. Disables options -f and -d")
    ('Z', "h_counts","","","The first arguments is a counts file with only highest order counts, not a text file to be read. Disables options -f and -d")
    ...
    ('r', "prune_with_real_counts", "", "", "Use real counts and not type counts when deciding which grams to use. Slightly better results, uses more memory.")
    ('R', "real_counts_data=FILE", "arg", "", "if -r is specified with -c or -Z, the original training data must be specified.")
    ('W', "write_counts=FILE", "arg", "", "Write resulting count matrices to FILE.")
    ('U', "write_vocab=FILE", "arg", "", "Write resulting vocabulary FILE.")
    //('u', "skip_unks", "arg", "", "Do not estimate any probability for unknow tokens")
  ...
  const bool prune_with_real_counts=config["prune_with_real_counts"].specified;
  ...
  if (counts_in && prune_with_real_counts && rcfile.size()==0) {
    fprintf(stderr,"Using counts from file: pruning with real counts needs -R traindata. Exit.\n"); 
    exit(-1);
  }
  if (rcfile.size()) prunedata_name=rcfile;
  else prunedata_name=dataname;
  TreeGram lm;
  ...
  try {
    ...
    if (!use_3nzer && !smallvocab) {
      ...
      if (!longint) kn=new InterKn_int_disc3<unsigned short, int>(absolute, dataname, vocabname, optiname, counts_in, n, ndrop, nfirst, NULL, prunedata_name, ss_sym, hashs);
      else kn=new InterKn_int_disc3<unsigned short, long>(absolute, dataname, vocabname, optiname, counts_in, n, ndrop, nfirst, NULL, prunedata_name, ss_sym, hashs);
    } 
    ...
    if (use_3nzer) fprintf(stderr,"modified ");
