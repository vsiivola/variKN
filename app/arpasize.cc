...
void simple_count(conf::Config &config, io::Stream &in, io::Stream &out)
{
  /**********************************************************************/
  // The simple vesion, just read the counts of the arpa file
  /***********************************************************************/
  if (!config["arpa"].specified) {
    fprintf(stderr,"Sorry, the --simple flag only works with arpa format models currently. Fix me.\n");
    exit(-1);
  }
  ...
  while (str::read_line(&sbuf,in.file,true) && sscanf(sbuf.c_str(),"ngram %d=%d",&i2,&count)==2) {
    if (i2!=i) {
...
void dummyless_count(TreeGram &ng, io::Stream &out) {
  ...
  for (int o=2;o<=ng.order();o++) {
	...
	continue;
      ...
      //fprintf(stderr,"lp %g, lpp %g, pdif %2.1f, ",lp_orig, lp_prefix, fabs((lp_orig-lp_prefix)/lp_orig));
...
int main (int argc, char **argv) {
  conf::Config config;
  config("Usage:  arpasize in.bin out\nShows the number of grams modeled in an arpa file. Grams added for bookkeeping are discarded from these counts.\n")
    ('S',"simple", "", "", "Simply use the reported counts and dont's check for dummy grams.")
