...
template <typename KT>
void HashGram_t<KT>::normalize_and_set_bo(
...
template <typename KT>
void HashGram_t<KT>::renormalize_backoffs(int order) {
        ...
        while (probs[order]->ordered_stepthrough(false, &gram[0], &gram_logprob)) {
                ...
                if (different) {
                        ...
                        normalize_and_set_bo(cur_prefix, explicit_probsum,
                                             full_bo_probsum);
        ...
        if (order>1) {
                ...
                // Redistribute the remaining bigram mass equally accross unigrams
                ...
                if (o <= other.order()) {
                        ...
                        while (other.probs[o]->stepthrough(false, &gram[0], &gram_logprob)) {
                                ...
                                probs[o]->setvalue(
                                        &gram[0],
                                        safelogprob(
                                                lambda*pow(10, log_prob_bo(gram)) +
