...
public:
  inline InterKn(
    const bool absolute_discounting, const std::string &dataname,
    ...
    m_absolute_discounting(absolute_discounting),
    m_data_name(dataname), m_opti_name(optiname), m_prunedata_name(prunesource)
  {}
  virtual ~InterKn() {}
...
class InterKn_k : public InterKn {
public:
  ...
  //virtual void prune_model(float treshold, bool recorrect_kn, Storage<KT> *real_counts)=0;
...
class InterKn_t : public InterKn_k<KT> {
public:
...
public:
  InterKn_int_disc(
    const bool abs, const std::string data, const std::string vocab,
  ...
  virtual inline void add_counts_for_backoffs() {
    InterKn_t<KT, ICT>::add_counts_for_backoffs_fbase((MultiOrderCounts_counter_types::bo_c<ICT> *) NULL);
  }
  virtual inline void add_zeroprob_grams() {
    InterKn_t<KT, ICT>::add_zeroprob_grams_fbase((MultiOrderCounts_counter_types::bo_c<ICT> *) NULL);
  }
...
public:
  ...
  virtual inline float kn_coeff(const int order, const KT *i) {
    return kn_coeff_3nzer(order, i, (MultiOrderCounts_counter_types::bo_3c<ICT> *) NULL);
  }
  int debug() {KT i=1;return(this->moc->GetBackoffDen(1,&i));}
  void print_matrix(int o) {if (o<this->moc->m_counts.size() && o>=1) this->moc->m_counts[o]->printmatrix();}
  virtual inline void remove_sent_start_prob() {
    remove_sent_start_prob_fbase((MultiOrderCounts_counter_types::bo_3c<ICT> *) NULL);
  }
  inline void disc2flatv(std::vector<float> &v);
  inline float flatv2disc(std::vector<float> &v);
  void set_order(int o);
  ...
  virtual inline void add_counts_for_backoffs() {
    this->add_counts_for_backoffs_fbase((MultiOrderCounts_counter_types::bo_3c<ICT> *) NULL);
  }
  virtual inline void add_zeroprob_grams() {
    this->add_zeroprob_grams_fbase((MultiOrderCounts_counter_types::bo_3c<ICT> *) NULL);
  }
  protected:
  virtual void prune_gram(std::vector<KT> &v, ICT num, bool recorrect_kn, MultiOrderCounts_counter_types::bo_3c<ICT> *dummy);
  template <typename BOT> float kn_coeff_3nzer(const int order, const KT *i, const BOT* dummy=NULL);
  template <typename BOT> void remove_sent_start_prob_fbase(BOT *dummy);
