...
struct matrix *CreateMatrix(const int keysize,
  ...
  case INT: m->size_of_entry=sizeof(int);break;
  case FLOAT: m->size_of_entry=sizeof(float);break;
  case DOUBLE: m->size_of_entry=sizeof(double);break;
  case UNKNOWN: m->size_of_entry=soe;
  }
  if (soe!=m->size_of_entry) {
    fprintf(stderr,"Conflicting memory requirement, type of entry %d, size of entry %d\nExit.\n",m->type_of_entry,(int) m->size_of_entry);
    exit(-1);
  }
  ...
  m->num_entries=0;
  m->datasize=hashs;
  //fprintf(stderr,"mallocing %d*%d=%d\n", (int) m->key_size, hashs, (int) (m->key_size *hashs));
  ...
  while (pr[++i]<hashs) {
    if (i==NUM_OF_PRIMES-1) {
      i=NUM_OF_PRIMES-1;
      fprintf(stderr,"Trying to create a too big hash.\nLimited the hash size to %d ! To be able to create bigger hashes, add a sufficiently big prime to sparse_matrix.h.\n",(int) pr[i]);
      //exit(-1);
    }
  }
  m->hashsize=pr[i];
...
indextype NewEntry(struct matrix *m) {
  ...
  */
  ...
  } else if (m->allow_shrinking && 
	     (m->num_entries<m->datasize/3 && m->num_entries>m->hashsize)) {
    m->datasize/=2;
    m->keys=(byte *) realloc(m->keys, m->datasize * m->key_size);
...
void RemoveEntryIdx(struct matrix *m, const indextype idx) {
  ...
  if (m->prev[idx]>=0) m->hash[m->prev[idx]] = m->next[idx];
  else m->next[-m->prev[idx]-1] = m->next[idx];
  ...
  if (idx==m->num_entries) return;

  /* Fix the next fields for the prev of entry that will be moved */
  if (m->prev[m->num_entries]>=0) m->hash[m->prev[m->num_entries]]=idx;
  else m->next[ - m->prev[m->num_entries] - 1 ] = idx;
  if (m->next[m->num_entries]>=0) m->prev[m->next[m->num_entries]]=-idx-1;
...
indextype FindEntry(struct matrix *m, const byte *key, const int create) {
  ...
  if (*storeidx<0) { /* There is no node here */
    if (create<=0) return(-1); /* Delete or read, nothing to do */
    *storeidx=NewEntry(m);
    m->next[*storeidx]=-1;
    ...
    const int cr=memcmp(&(m->keys[*storeidx * m->key_size]), key, m->key_size);
    if (cr >0){ 
      ...
      *storeidx=NewEntry(m);
      ...
      idx=*storeidx;
      while (m->next[idx]>=0 && (memcmp(&(m->keys[m->next[idx]*m->key_size]),
					key, m->key_size)<=0)) {
	idx=m->next[idx];
      }
      if (!memcmp(&(m->keys[idx*m->key_size]), key, m->key_size)) {
	/* Ok, found and return */
	if (create >= 0)  return (idx);
  ...
  memcpy(&(m->data[idx*m->size_of_entry]),m->default_value,m->size_of_entry);
  return(idx);
}

struct matrix *CreateMatrixF(const int d,  
			     const indextype hash, const float def_value){
  return(CreateMatrix(sizeof(int)*d,hash,(void *) &def_value,
				  FLOAT,sizeof(float)));
}

struct matrix *CreateMatrixI(const int d, 
			     const indextype hash, const int def_value) {
  return(CreateMatrix(sizeof(int)*d,hash,(void *) &def_value,
				  INT,sizeof(int)));  
}

struct matrix *CreateMatrixD(const int d, 
			     const indextype hash, const double def_value){
  return(CreateMatrix(sizeof(int)*d,hash,(void *) &def_value,
...
int IncrementI(struct matrix *m, const int *indices, const int value) {
  ...
  if ((*v += value ) != *((int *) (m->default_value))) return(*v);
...
float IncrementF(struct matrix *m, const int *indices, const float value) {
  ...
  if ((*v += value ) != *((float *) (m->default_value))) return(*v);
...
void *StepThrough(struct matrix *mat, byte *key, void *data) {
  ...
  if (++sm_STidx>=sm_STm->num_entries) return(NULL);
    
  /* Return indices */
  memcpy(key,&(sm_STm->keys[sm_STidx*sm_STm->key_size]),sm_STm->key_size);

  /* Return value */
  memcpy(data,&(sm_STm->data[sm_STidx*sm_STm->size_of_entry]),sm_STm->size_of_entry);
...
void CheckConsistency(struct matrix *m) {
  ...
  for (i=0;i< m->num_entries; i++) {
	...
	fprintf(stderr,"m_next[i] %d >= %d. i=%d. exit\n",
		(int) m->next[i], (int) m->num_entries,i);
	exit(-1);
      }
      pn = m->prev[m->next[i]];
      assert(pn<0);
	...
	fprintf(stderr,"-mp-1 %d, mn[-mp-1] %d, i %d. exit.\n",
		(int) (- m->prev[i]-1), (int) m->next[- m->prev[i] -1],
		(int) i);
	exit(-1);
      }
    }
...
void showvalues(struct matrix *m, indextype i) {
  int a=-999;
  fprintf(stderr,"%d: prev%d next%d, max%d, ",(int) i, (int) m->prev[i], (int) m->next[i], (int) m->num_entries);
  if (m->prev[i]>=0) a=m->hash[m->prev[i]];
  else a =m->next[-m->prev[i]-1];
  fprintf(stderr,"consistency p%d ",(int) a);
  if (m->next[i]>0) fprintf(stderr,"n%d ",(int) (-m->prev[m->next[i]]-1));
  fprintf(stderr,"\n");
}
...
void RemoveDefaultValues(struct matrix *m) {
  indextype i;
  for (i=0; i<m->num_entries; i++) {
    if (!memcmp(&(m->data[i*m->size_of_entry]), 
		m->default_value, m->size_of_entry)) 
      RemoveEntryIdx(m, i);
  }
}
...
#include "sparse_matrix_pinline.c"
#endif
