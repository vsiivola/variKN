...
class Varigram_t : public Varigram {
public:
  Varigram_t(bool use_3nzero, bool absolute=false);
  ~Varigram_t();
  void initialize(std::string infilename, indextype hashsize, int ndrop, int nfirst,
...
private:
  ...
  double modify_model(std::map<KT, ICT> &new_c, const std::vector<KT> &v, const float ml_norm);
