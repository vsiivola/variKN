...
template <typename KT, typename CT>
long MultiOrderCounts<KT, CT>::InitializeCountsFromStorage(Storage_t<KT, CT> *data, const int read_order, const int sent_start_idx) {
  long num_read=0;
  std::vector<KT> v;
  KT idx;
...
template <typename KT, typename CT>
CT MultiOrderCounts<KT, CT>::IncrementCountCache(
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffCache(const int order, const KT *indices, const BOT *value) {
...
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::UndoCached() {
  ...
  */
  ...
  for (long i=bo_cache.size()-1;i>=0;i--) {
    ...
    if (c.index >= min_bo_cache[c.order]) continue;
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoff(const int order, const KT *v, const BOT *value) {
  if (order==1) {
    m_uni_bo+= *value;
    return;
  }
...
template <typename KT, typename CT, typename BOT>
CT MultiOrderCounts_Generic_BOT<KT, CT, BOT>::GetBackoffDen(const int order, const KT *v) {
  if (order==1) return(m_uni_bo.den);
...
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::RemoveOrder(int order) {
  fprintf(stderr,"cur_o %d(%ld): ",order, (long) MultiOrderCounts<KT, CT>::m_counts.size()-1);
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoff(const std::vector<KT> &v, const BOT *value) {
  IncrementBackoff(v.size(),&v[0],value);
}

template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::SetBackoff(
  const int order, const KT *v, const BOT *bo) {
  allocate_matrices_backoffs(order);
  if (order>1) {
...
MultiOrderCounts_Generic_BOT<KT, CT, BOT>::MultiOrderCounts_Generic_BOT() 
{
  ...
  m_3bb_init.den=0; 
  m_3bb_init.nzer[0]=0;m_3bb_init.nzer[1]=0;m_3bb_init.nzer[2]=0;
  m_3bb_init.prune_den=0;
  m_3bb_fp_init.den=0;
  m_3bb_fp_init.nzer[0]=0;m_3bb_fp_init.nzer[1]=0;m_3bb_fp_init.nzer[2]=0;
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffDen(const int order, const KT *indices, const CT den) {
  allocate_matrices_backoffs(order);
  if (order>1) {
    struct matrix *m = m_backoffs[order]->m;
    indextype idx=FindEntry(m, (byte *) indices,1);
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::allocate_matrices_backoffs(int o
) {
...
void MultiOrderCounts_1nzer_fp<KT, CT>::clear_lden(const int o) {
  if (o>1) {
    for (indextype i=0;i<MultiOrderCounts_Generic_BOT<KT, CT,  MultiOrderCounts_counter_types::bo_c_fp<CT> >::m_backoffs[o]->num_entries();i++) {
      MultiOrderCounts_counter_types::bo_c_fp<CT> *bop =
	MultiOrderCounts_Generic_BOT<KT, CT,  MultiOrderCounts_counter_types::bo_c_fp<CT> >::m_backoffs[o]->Idx2Valp(i);
      bop->lost_den=0;
    }
    return;
  }
  MultiOrderCounts_Generic_BOT<KT, CT,  MultiOrderCounts_counter_types::bo_c_fp<CT> >::m_uni_bo.lost_den=0;
}

template <typename KT, typename CT, typename BOT>
void *MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
StepBackoffsOrder(const bool init, const int order, KT *indices, BOT *value) {
  if (order>=MultiOrderCounts<KT, CT>::m_counts.size()) return(NULL);
  assert(order>=2);
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffNzer_1nzer(const int order, const KT *indices, const CT nz) {
  if (order>1) {
    allocate_matrices_backoffs(order);
    struct matrix *m=m_backoffs[order]->m;
    indextype idx=FindEntry(m,(byte *) indices,1);
...
template <typename KT, typename CT, typename BOT>
CT MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
GetBackoffNzer_1nzer(const int order,const KT *v){
  if (order==1) return(m_uni_bo.nzer);
  if (order>=m_backoffs.size()) return(0);
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffCacheNzer_1nzer(const int order, const KT *v, const CT value) {
  BOT bo;
  zero_bo(bo);
  bo.nzer=value;
  IncrementBackoffCache(order,v, &bo);
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
GetBackoffNzer_3nzer(const int order, const KT *v, CT *res) {
  if (order==1) {
    for (int i=0;i<3;i++)
...
template <typename KT, typename CT, typename BOT>
CT MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
GetBackoffNzer_3nzer(const int order, const KT *v, const int which) {
  CT res[3];
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffCacheNzer_3nzer(const int order, const KT *v, const CT *value){
  BOT bo;
  zero_bo(bo);
  bo.nzer[0]=value[0];bo.nzer[1]=value[1];bo.nzer[2]=value[2];
  IncrementBackoffCache(order,v, &bo);
}

template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffCacheNzer_3nzer(const int order, const KT *v, const int pos, 
				const CT value){
  BOT bo;
  zero_bo(bo);
  bo.nzer[pos]=value;
...
template <typename KT, typename CT, typename BOT>
void MultiOrderCounts_Generic_BOT<KT, CT, BOT>::
IncrementBackoffNzer_3nzer(const int order, const KT *indices, const int pos, 
			   const CT nz) {
  if (order > 1) {
    struct matrix *m=m_backoffs[order]->m;
    indextype idx=FindEntry(m,(byte *) indices,1); 
