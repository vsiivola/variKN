...
float InterKn_int_disc<KT, ICT>::kn_prob(const int order, const KT *i, const ICT num)
{
  ...
  if (order==1) {
    ...
    prob = (m_discount[1]*this->moc->GetBackoffNzer(1,NULL))
      /(float) this->moc->GetBackoffDen(1,NULL)/((float) this->vocab.num_words());
    //fprintf(stderr,"Unigram smooth %.4f\n", prob);
  }

  if (num<=0) return(prob);
...
float InterKn_int_disc3<KT, ICT>::kn_prob(const int order, const KT *i, const ICT num){
  ...
  if (order==1) {
    ...
    prob=(m_discount[1][0]*bo.nzer[0]
          +m_discount[1][1]*bo.nzer[1]
...
template <typename KT, typename CT> template <typename BOT> 
void InterKn_t<KT, CT>::prune_model_fbase
(float treshold, bool recorrect_kn, Storage_t<KT, CT> *real_counts, BOT *dummy) {
  std::vector<KT> v;
  CT num;
  ...
  for (int o=this->order();o>=2;o--) {
	...
	if (flag) {
      ...
      if (real_counts) {
	const indextype idx = FindEntry(moc->m_counts[o]->m, (byte *) (&v[0]), 0);
    ...
    if (real_counts) {
      moc->StepCountsOrder(true,o,&v[0],&num);
      while (moc->StepCountsOrder(false,0,&v[0],&num)) {
	if (num==0) moc->DeleteCurrentST(o);
      }
    }
  }
  // Clean backoffs 
...
void InterKn_int_disc3<KT, ICT>::prune_gram(
  ...
  bo.nzer[0]=0;bo.nzer[1]=0; bo.nzer[2]=0;
  ...
  if (recorrect_kn && num!=1) { 
    ...
    if (origl == num -1) {
      ...
      bo.den=num-1;bo.nzer[0]=0;bo.nzer[1]=0;bo.nzer[2]=0;bo.prune_den=0;
...
void InterKn_t<KT, CT>::counts2asciilm(FILE *out) {
  ...
  if (this->zeroprobgrams) add_zeroprob_grams();
  ...
  for (int o=1;o<=this->m_order;o++) {
    ...
    while (moc->StepCountsOrder(false,o,&gram[0],&num)) {
      ...
      if (!this->zeroprobgrams && (logprob<=MINLOGPROB && bo_den<=0)) continue;
...
void InterKn_t<KT, CT>::constructor_helper(
  const std::string &vocabname, const int read_counts, const int order, const int ndrop, 
  const int nfirst_i, Storage_t<KT, CT> *datastorage, const indextype hashsize, 
  const std::string &sent_start_symbol){

  int nfirst=nfirst_i;
  if (nfirst==-1) nfirst=9999999;
  ...
  if (read_counts) {
	...
	fprintf(stderr,"Warning: something is going wrong. The indices must match, this will not be checked.\n");
...
double InterKn_t<KT, CT>::logprob_datastorage(const Storage<KT> &data) {
  ...
  if (this->m_sent_boundary==-1) {
    for (size_t j=0;j<data.size();j++) {
      if (target<this->m_order) {target++;indices.resize(target);}
...
void InterKn_t<KT, CT>::estimate_bo_counts(bool zero_sentstart) {
  ...
  for (int o=this->m_order;o>=1;o--) {
	...
	moc->IncrementCount(o-1,&v[1],std::min(value,m_new_treshold));
      ...
      if (!flag) moc->IncrementBackoffDen(o,&v[0],value);
    }
  }
  remove_zeroprob_grams();

  if (zero_sentstart) remove_sent_start_prob();
}

template <typename KT, typename CT>
void InterKn_t<KT, CT>::estimate_bo_counts_absolute_discounting(bool zero_sentstart) {
  /* Estimate backoff and lower order counts            */
  std::vector<KT> v(this->m_order);
  CT value;
