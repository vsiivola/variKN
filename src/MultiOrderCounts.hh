...
// Different smoothing schemes defined here (bo_*)
  ...
  struct bo_3c_fp {
    ...
    bo_3c_fp operator+=(const bo_3c_fp other) { // defined for sikMatrix.increment()
      den+=other.den;
      nzer[0]+=other.nzer[0];
      nzer[1]+=other.nzer[1];
      nzer[2]+=other.nzer[2];
...
public:
  ...
  virtual void GetBackoff(const int order, const KT *v, BOT *value){assert(false);}
  virtual void IncrementBackoffCache(const int order, const KT *v, const BOT *value){assert(false);}
  virtual void IncrementBackoff(const int order, const KT *v, const BOT *value) {assert(false);}
  virtual void IncrementBackoff(const std::vector<KT> &v, const BOT *value) {assert(false);}
  virtual void *StepBackoffsOrder(const bool init, const int order, 
    KT *indices, BOT *value) {assert(false);return(NULL);}
  virtual void zero_bo(BOT &z){assert(false);}
};

template <typename KT, typename CT>
class MultiOrderCounts :
  public MultiOrderCounts_typed_interfaces<KT, CT, MultiOrderCounts_counter_types::bo_c<CT> >,
  public MultiOrderCounts_typed_interfaces<KT, CT, MultiOrderCounts_counter_types::bo_c_fp<CT> >,
...
public:
  ...
  virtual void IncrementBackoffCacheNzer(const int order, const KT *v, 
					 const CT *value){assert(false);}
  virtual void IncrementBackoffCacheNzer(const int order, const KT *v,
    const int pos, const CT value) {assert(false);}
...
public:
  ...
  void *StepBackoffsOrder(const bool init, const int order, KT *indices, BOT *value);
  ...
  inline void IncrementBackoffCacheNzer_3nzer(const int order, const KT *v,
					      const int pos, const CT value);
  inline void zero_bo(typename MultiOrderCounts<KT,CT>::bo_c &z) {z=m_bb_init;}
  inline void zero_bo(typename MultiOrderCounts<KT,CT>::bo_c_fp &z) {z=m_bb_fp_init;}
  inline void zero_bo(typename MultiOrderCounts<KT,CT>::bo_3c &z) {z=m_3bb_init;}
  inline void zero_bo(typename MultiOrderCounts<KT,CT>::bo_3c_fp &z) {z=m_3bb_fp_init;}
...
protected:
  ...
  inline void zero_nz(typename MultiOrderCounts<KT,CT>::bo_c *z) {z->nzer=0;}
  inline void zero_nz(typename MultiOrderCounts<KT,CT>::bo_c_fp *z){z->nzer=0;}
  inline void zero_nz(typename MultiOrderCounts<KT,CT>::bo_3c *z) {
    z->nzer[0]=0;z->nzer[1]=0;z->nzer[2]=0;}
...
template <typename KT, typename CT>
class MultiOrderCounts_1nzer : public MultiOrderCounts_Generic_BOT
...
template <typename KT, typename CT>
class MultiOrderCounts_1nzer_fp : public MultiOrderCounts_Generic_BOT
...
template <typename KT, typename CT>
class MultiOrderCounts_3nzer : public MultiOrderCounts_Generic_BOT<KT, CT, typename MultiOrderCounts<KT,CT>::bo_3c> {
public:
...
private:
  inline void WriteCounts_BOhelper(FILE *out, bo_3c *bo) {
    fprintf(out, "%ld %ld %ld %ld %ld", 
	    (long) bo->nzer[0], (long) bo->nzer[1], (long) bo->nzer[2], (long) bo->den, 
	    (long) bo->prune_den);
  }
...
template <typename KT, typename CT>
class MultiOrderCounts_3nzer_fp : public MultiOrderCounts_Generic_BOT<KT, CT, typename MultiOrderCounts<KT,CT>::bo_3c_fp> {
