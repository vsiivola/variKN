...
std::vector<float> QFit::minimize(int maxiter) {
  //fprintf(stderr, "vsizes %zd %zd %zd\n", m_initial_point.size(), m_low_limits.size(), m_high_limits.size());
  ...
  while (iter<maxiter) {
	...
	if (t2>=t1 || t2>=t3) {
	  ...
	  //fprintf(stderr,"qfit\n");
	  l = (-(t3-t2)*(l1*l1-l2*l2)+(t1-t2)*(l3*l3-l2*l2))/
	    ((t1-t2)*(l3-l2)-(t3-t2)*(l1-l2))/2;
	  //fprintf(stderr, "(-(t3 %g-t2 %g)*(l1 %g*l1-l2 %g*l2)+(t1 %g-t2 %g)*(l3%g*l3-l2 %g*l2))/((t1 %g-t2 %g)*(l3 %g-l2 %g)-(t3 %g-t2 %g)*(l1 %g-l2 %g))/2=l %g\n", t3, t2, l1, l2, t1,t2,l3,l2,t1,t2,l3,l2,t3,t2,l1,l2,l);
	  m_res[c]=l;
...
void QFit::check_limits(int biter, int c, float &l1, float &l2, float &l3,
  ...
  if (m_sslim.size()) { // if initial limits for the search are given
    ...
    if (l1 < m_res[c]- m_sslim[c]) l1 = m_res[c] -m_sslim[c];
    if (l3 > m_res[c] + m_sslim[c]) l3 = m_res[c] +m_sslim[c];
  }

  if (l2-l1<SMALLNUM) l2=l1+SMALLNUM; 
  if (l3-l2<SMALLNUM) l2=l3-SMALLNUM; 

  //fprintf(stderr,"init_limits %g<%g<%g\n", m_low_limits[c],m_res[c],m_high_limits[c]);
  //fprintf(stderr,"start_limits %g<%g<%g\n", l1,l2,l3);
  while (true) {
    m_res[c]=l1;
