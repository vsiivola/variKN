...
sikMatrix<K, V>::sikMatrix(const int dims_in, const indextype hashsize, 
  ...
  //fprintf(stderr,"CreateMatrix with %d %d %d\n", sizeof(K), dims, (int) hashsize);
  m = CreateMatrix(sizeof(K)*dims, hashsize, 
		   (void *) &def_value, UNKNOWN, sizeof(V));
  stept_sortsearch=false;
}

template <typename K, typename V>
sikMatrix<K, V>::~sikMatrix() {
  DeleteMatrix(m);
}
...
V sikMatrix<K, V>::increment(const K *indices, const V &value) {
  ...
  *v += value;
  if (memcmp(v,m->default_value,m->size_of_entry)) return(*v);
...
void *sikMatrix<K, V>::stepthrough(const bool init, K *indices, V *value) {
  if (!init) return(StepThrough(NULL, (byte *) indices, (void *) value));
...
void *sikMatrix<K, V>::ordered_stepthrough(const bool init, K *indices, 
					   V *value) {
  if (!init) return(OrderedStepThrough(NULL, (byte *) indices,(void *) value));
  return(OrderedStepThrough(m, (byte *) indices, (void *) value));
}

template <typename K, typename V>
void *sikMatrix<K, V>::stept(const bool init, K *indices, V *value) {
  if (stept_sortsearch) return(ordered_stepthrough(init, indices, value));
  return(stepthrough(init, indices, value));
}

template <typename K, typename V>
K *sikMatrix<K, V>::Idx2Keyp(indextype idx) {
  return((K *) &(m->keys[idx*m->key_size]));
}

template <typename K, typename V>
V *sikMatrix<K, V>::Idx2Valp(indextype idx) {
...
#include "def.hh"
template <typename K, typename V>
void sikMatrix<K, V>::printmatrix() {
