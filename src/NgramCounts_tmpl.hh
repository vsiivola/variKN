...
NgramCounts_t<K, T>::NgramCounts_t(const int n, const int max_vocab, indextype hashsize)
{
  if (n<=0) {
    fprintf(stderr,"Impossible n (%d). Exit.\n",n);
    exit(-1);
  }
  if (max_vocab) m_max_vocab=max_vocab;
  else m_max_vocab=5000000;

  for (int i=0;i<n;i++) 
    m_ds.push_back(m_max_vocab);
  
  if (!hashsize) hashsize=6000000;
  counts=new sikMatrix<K, T>(n,hashsize,0);
}

template <typename K, typename T>
NgramCounts_t<K, T>::~NgramCounts_t() {
  delete counts;
}
...
long NgramCounts_t<K, T>::count(FILE *file, bool grow_vocab) {
  ...
  while (fscanf(file,MAX_WLEN_FMT_STRING,charbuf)!=EOF) {
    ...
    if (idx>=m_max_vocab-1) {
      fprintf(stderr,"Exceeded maximum vocab size %d.\n"
...
long NgramCounts_t<K, T>::count(Storage_t<K, T> *data) {
  ...
  for (size_t di=0; di<data->size(); di++) {
    ...
    if (idx>=m_max_vocab-1) {
      fprintf(stderr,"Datastorage has different vocab than ngramcounts. Exit.\n");
      exit(-1);
    }
...
void NgramCounts_t<K, T>::write(FILE *out, FILE *vocabout, bool sort) {
  ...
  if (vocabout) vocab->write(vocabout);
}
...
void NgramCounts_t<K, T>::shrink(float ndrop, int nfirst) {
  if (nfirst==0) nfirst=INT_MAX;
  if (vocab->num_words()<nfirst && ndrop<=0) return;
  ...
  for (int i=0;i<vocab->num_words();i++) wordtable[i].wstring=vocab->word(i);
  /* Get the word counts */
  long num_grams=0;
  ...
  /* Redo the counts */
  for (i=0;i<order();i++) v[i]=nfirst+1;
  sikMatrix<K, T> *new_counts=new sikMatrix<K, T>(i,std::min(num_grams+1,(long) 10000019),0);
  counts->stepthrough(true,&v[0],&count);
  while (counts->stepthrough(false,&v[0],&count)) {
    for (int i=0;i<order();i++) v[i]=backwards[v[i]];
