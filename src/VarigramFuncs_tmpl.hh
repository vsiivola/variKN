...
void Varigram_t<KT, ICT>::printmatrix_bo(sikMatrix<KT, typename MultiOrderCounts<KT, ICT>::bo_3c> *m) {
...
template <typename KT, typename ICT>
void Varigram_t<KT, ICT>::printmatrix_bo(sikMatrix<KT, typename MultiOrderCounts<KT, ICT>::bo_c> *m) {
...
template <typename KT, typename ICT>
void Varigram_t<KT, ICT>::
initialize(std::string infilename, indextype hashsize, int ndrop, int nfirst, 
...
void Varigram_t<KT, ICT>::grow(int iter2_lim) {
  ...
  while (iter2<iter2_lim) {
	...
	if (new_history.size()!=old_hist_size) {
	  ...
	  if (!m_small_memory) 
	    m_data->initialize_fast_search_lists(new_history.size()+1, curref1, curref2);
	  else 
	    m_data->init_fsl_file(new_history.size()+1, curref1, m_infilename, m_vocab);
	}
...
template <typename KT, typename ICT>
double Varigram_t<KT, ICT>::modify_model(
  std::map<KT,ICT> &m, const std::vector<KT> &indices, const float ml_norm) {
  ICT val;
  int order=indices.size()+1;
  double logprobdelta=0.0;

  /* Check, if m_kn->m_order should be increased */
  if (m_kn->order() <order) m_kn->set_order(order);
...
//	printmatrix_bo(moc->m_backoffs[order]);
	...
	printmatrix_bo(moc->m_backoffs[order]);
      ...
      /* The nonzero counts for new order */
      moc->IncrementBackoffCacheNzer(order,&ind[0],std::min(val-1,(ICT) 2),1); 
      // DEBUGGGG
      //printmatrix_bo(moc->m_backoffs[order]);
...
template <typename KT, typename ICT>
void Varigram_t<KT, ICT>::get_unigram_counts(std::string &infilename, int ndrop, int nfirst, int *type){
...
template <typename KT, typename ICT>
void Varigram_t<KT, ICT>::get_unigram_counts(std::string &infilename, int ndrop, int nfirst, unsigned short *type){
  io::Stream in;
