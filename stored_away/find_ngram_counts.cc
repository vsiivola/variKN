...
int main(int argc, char **argv) {
  conf::Config config;
  config("Usage: find_ngram_counts [OPTIONS] textin countsout\nExtracts n-gram counts from given text.\n")
    ('n', "norder=INT", "arg must", "3", "n-gram order")
    ('B', "vocabin=FILE", "arg", "", "specify vocabulary")
    ('b', "vocabout=FILE", "arg", "", "if chosen, a vocabulary file will be written and the output count file will only contain the word indices")
    ('H', "hashsize=INT", "arg", "0", "size of the reserved hash table")
  ...
  if (sort) fprintf(stderr," Will sort n-grams\n");
